...
//        private val gson = Gson()
        ...
        fun updateHomeScreenWidget(context: Context, tasks: List<Task>, startTime: LocalDateTime, endTime: LocalDateTime) {
            ...
            SystemStatus.logEvent("TaskWidget", "No widgets found to update")
            return
        }
            Log.d("TaskWidgetProvider", "Found ${appWidgetIds.size} widget IDs: ${appWidgetIds.joinToString()}")
        ...
        SystemStatus.logEvent("TaskWidget", "Failed to update widget: ${e.message}")
        Log.e("TaskWidgetProvider", "Widget update failed", e)
    }
        }
        ...
        private fun startAnimations(context: Context) {
            ...
            // Use safer scheduling mechanism
            timer = Timer().apply {
                try {
                    schedule(object : TimerTask() {
                        override fun run() {
                            synchronized(this) {
                                progress = (progress + 0.1f) % 3600f
                                snakePosition = (snakePosition + 2) % 100
                                snakeActive = !snakeActive
                    ...
                    }, 200, 500) // Increase interval to 500ms
                } catch (e: Exception) {
                    SystemStatus.logEvent("TaskWidget", "Failed to start animations: ${e.message}")
                }
            }
        }
