...
class SubTaskAdapter(
    ...
    private val onCompletionChange: (SubTask, Any, Int) -> Unit
) : RecyclerView.Adapter<SubTaskAdapter.SubTaskViewHolder>() {

    class SubTaskViewHolder(val binding: ItemLockSubtaskBinding) : RecyclerView.ViewHolder(binding.root)

    fun getSubTaskAt(position: Int): SubTask? = if (position in subTasks.indices) subTasks[position] else null
            ...
            when (subTask.measurementType) {
                ...
                "time" -> {
                    ...
                    val label = TextView(root.context).apply {
                        text = "Time Spent: $timeSpent/$setDuration minutes"
                ...
                "quant" -> {
                    ...
                    val label = TextView(root.context).apply {
                        text = "Achieved: $achievedValue/$targetValue ${subTask.quant?.targetUnit}"
                        setTextAppearance(android.R.style.TextAppearance_Medium)
                        layoutParams = LinearLayout.LayoutParams(
                            LinearLayout.LayoutParams.MATCH_PARENT,
                            LinearLayout.LayoutParams.WRAP_CONTENT
                ...
                "deepwork" -> {
                    inputLayout.orientation = LinearLayout.VERTICAL
                    val isCompleted = subTask.completionStatus == 1.0f
                    val cameraButton = Button(root.context).apply {
                        text = "Open Camera"
                        layoutParams = LinearLayout.LayoutParams(
                            LinearLayout.LayoutParams.MATCH_PARENT,
                            48.dpToPx(root.context)
                        ...
                        isEnabled = !isCompleted
                        setBackgroundColor(if (isCompleted) Color.parseColor("#B0BEC5") else Color.parseColor("#BB86FC"))
                        setTextColor(Color.WHITE)
                        setOnClickListener {
                            onCompletionChange(subTask, true, position)
                        }
                    }
                    val ocrResult = TextView(root.context).apply {
                        text = subTask.deepwork?.template?.let { "OCR Result: ${subTask.deepwork?.template}" } ?: "No OCR result yet"
