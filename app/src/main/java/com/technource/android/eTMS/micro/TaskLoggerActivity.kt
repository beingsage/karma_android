...
import com.technource.android.utils.CameraActivity
...
class TaskLoggerActivity : AppCompatActivity() {
    ...
    private var currentSubTaskPosition: Int? = null

    private val cameraResultLauncher = registerForActivityResult(ActivityResultContracts.StartActivityForResult()) { result ->
        if (result.resultCode == RESULT_OK) {
            val data = result.data
            val extractedText = data?.getStringExtra(CameraActivity.EXTRA_EXTRACTED_TEXT) ?: ""
            val shareableLink = data?.getStringExtra(CameraActivity.EXTRA_SHAREABLE_LINK) ?: ""
            currentSubTaskPosition?.let { position ->
                val subTask = (binding.subTasksRecycler.adapter as? SubTaskAdapter)?.getSubTaskAt(position)
                subTask?.let {
                    it.deepwork?.apply {
                        template = shareableLink
                        deepworkScore = 0
                    }
                    it.completionStatus = 1.0f
                    it.finalScore = it.baseScore.toFloat()
                    (binding.subTasksRecycler.adapter as? SubTaskAdapter)?.notifyItemChanged(position)
                    updateTaskCompletion()
                }
            }
        }
        currentSubTaskPosition = null
    }

    private val loggerTimeoutMs = 60_000L // 1 minute
    private var loggerTimedOut = false
    private var userLoggedTask = false
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        currentTask = task // Save reference for timeout
        ...
        // Start timeout for auto-miss
        handler.postDelayed({
            if (!userLoggedTask && !loggerTimedOut) {
                loggerTimedOut = true
                markTaskAsMissedAndClose()
            }
        }, loggerTimeoutMs)
    }
    ...
    private fun setupUI(task: Task) {
            ...
            adapter = SubTaskAdapter(task.subtasks ?: emptyList()) { subTask, completionData, position ->
                when (subTask.measurementType) {
                    ...
                    "deepwork" -> {
                        ...
                        val intent = Intent(this@TaskLoggerActivity, CameraActivity::class.java)
        ...
        binding.logTaskButton.setOnClickListener {
            userLoggedTask = true // Mark that user interacted
            ...
            task.subtasks?.let { subtasks ->
                ...
                CoroutineScope(Dispatchers.IO).launch {
                    task.status = TaskStatus.LOGGED
                    taskDao.updateTask(task.toTaskEntity())
                    withContext(Dispatchers.Main) {
                        SystemStatus.logEvent(
                            "TaskLoggerActivity",
                            "Task ${task.title} saved with completion status ${task.completionStatus}, score ${task.taskScore}, status ${task.status}"
                        )
                        setResult(RESULT_OK)
                        finish()
                    }
    ...
    private fun markTaskAsMissedAndClose() {
        CoroutineScope(Dispatchers.IO).launch {
            currentTask.status = TaskStatus.MISSED
            taskDao.updateTask(currentTask.toTaskEntity())
            withContext(Dispatchers.Main) {
                SystemStatus.logEvent(
                    "TaskLoggerActivity",
                    "Task ${currentTask.title} marked as MISSED due to inactivity"
                )
    ...
    private fun updateTaskCompletion() {
        ...
        task.taskScore = task.subtasks?.sumOf { it.finalScore.toDouble() }?.toFloat() ?: 0.0f
        binding.taskProgress.progress = (task.completionStatus * 100).toInt()
    }
}
